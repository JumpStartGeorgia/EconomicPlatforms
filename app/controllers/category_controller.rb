	...
	def index
      ...
      if params[:political_party_id] && !params[:political_party_id].empty?
        ...
        params[:indicator_category_id] = 5
			...
			@platforms = Platform.published.by_election(@current_election_id).by_economic_category(@economic_category.id)
			@policy_briefs = PolicyBrief.published.by_election(@current_election_id).by_economic_category(@economic_category.id)

			gon.category_profile = true
      gon.category_statement_chart_data = true
      gon.json_data = Statement.sorted.category_statement_scores_json(
        @current_election_id,
